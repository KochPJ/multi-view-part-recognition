...
import copy
import math
from models.fusion import MaxPool
...
def get_args_parser():
    ...
    # training
    parser.add_argument('--name', default='ResNet_50_nr1-a_9', type=str) #_WeightNet
    parser.add_argument('--outdir', default='./results/run009', type=str)
    ...
    parser.add_argument('--force_views', default='1-6-9', type=str) #1-6-9
...
def main(args):
    ...
    print('Focring views {} for {} data views and {} views'.format(args.force_views, args.data_views, args.views))
    args.data_views = args.force_views
    args.views = args.force_views
    if len(args.views.split('-')) > 1:
        args.multiview = True
    n_views = len(args.views.split('-'))

    print(args.data_views,  args.views, args.multiview )
    ...
    losses = []
    t_step = time.time()
    fusion = MaxPool(num_views=n_views)
    ...
    for step, (x, y) in enumerate(dataloader):
        ...
        with torch.no_grad():
            pred = fusion(torch.cat([model(x['x'][:, i]) for i in range(n_views)]).unsqueeze(0))
