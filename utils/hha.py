...
if __name__ == "__main__":
    ...
    cls = 'bosch_22943691'
    p = '/home/kochpaul/git/vanishing-depth-self-supervised/MVIP/sets/{}/train_data/0/0/cam_10/10_depth.png'.format(cls)
    p2 = '/home/kochpaul/git/vanishing-depth-self-supervised/MVIP/sets/{}/train_data/0/0/cam_10/10_rgb.png'.format(cls)
    from PIL import Image
    D = np.array(Image.open(p)) / 1000
    camera_matrix = np.array([912.7331848305221, 0.0, 641.9770404469256, 0.0,
                              912.043533085843, 362.88386799435466, 0.0, 0.0, 1.0]).reshape((3, 3))
    ...
    print(type(hha), hha.shape)
    print(time.time()-t)
    hha_t = tf(hha)
    import torch
    print(type(hha_t), hha_t.shape, torch.mean(hha_t), torch.min(hha_t), torch.max(hha_t))

    import matplotlib.pyplot as plt
    plt.subplot(1, 3, 1)
    plt.imshow(Image.open(p2))
    plt.subplot(1, 3, 2)
    plt.imshow(hha)
    plt.subplot(1, 3, 3)
    plt.imshow(D)
